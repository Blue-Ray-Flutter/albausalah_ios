...
import '../model/store_reviews_model.dart';
import '../model/store_svervice_model.dart';

class StorePageController extends GetxController {
  final HttpRepository httpRepository;
  final CacheUtils cacheUtils;

  StorePageController({required this.httpRepository, required this.cacheUtils});
  ...
  Rx<StoreCategoriesModel?> storeCategoriesModel =
      Rx<StoreCategoriesModel?>(null);
  ...
  RxInt selectedValueStoreType = RxInt(1);
  ...
  getProducts({String? search}) async {
    ...
    try {
      productsResponse.value = await httpRepository.showProducts(
  ...
  getStoreCategories() async {
    late Rx<Response?> storeCategoriesResponse = Rx<Response?>(null);

    try {
      storeCategoriesResponse.value = await httpRepository.getStoreCategories(
        storeId: storeId.value,
      );

      if (storeCategoriesResponse.value == null) {
        return null;
      }
      storeCategoriesModel.value =
          StoreCategoriesModel.fromJson(storeCategoriesResponse.value!.body);
    } catch (e) {
      Get.snackbar(
        'Get Categories'.tr,
        'Something went wrong'.tr,
